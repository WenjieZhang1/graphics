...
//
//		Complete the Blinn-Phong lighting model: add emissive and specular:
...
//	
  ...
  '	 vec3 speculr = u_Lamp0Spec * u_Ks * e64;\n' +
        ...
        // Calculate the light direction vector, make it unit-length (1.0).
  '  vec3 lightDirection1 = normalize(u_Lamp0Pos1.xyz - v_Position.xyz);\n' +
      // The dot product of the light direction and the normal
      // (use max() to discard any negatives from lights below the surface)
      // (look in GLSL manual: what other functions would help?)
  ...
  '  float e641 = e321*e321; \n' +
    // Can you find a better way to do this? SEE GLSL 'pow()'.
      // Calculate the final color from diffuse reflection and ambient reflection
  '  vec3 emissive1 = u_Ke1;' +
  '  vec3 ambient1 = u_Lamp0Amb1 * u_Ka1;\n' +
  '  vec3 diffuse1 = u_Lamp0Diff1 * v_Kd * nDotL1;\n' +
  '  vec3 speculr1 = u_Lamp0Spec1 * u_Ks1 * e641;\n' +
...
function main() {
  ...
  var u_Lamp0Pos1  = gl.getUniformLocation(gl.program,   'u_Lamp0Pos1');
  var u_Lamp0Amb1  = gl.getUniformLocation(gl.program,   'u_Lamp0Amb1');
  var u_Lamp0Diff1 = gl.getUniformLocation(gl.program,   'u_Lamp0Diff1');
  var u_Lamp0Spec1 = gl.getUniformLocation(gl.program,   'u_Lamp0Spec1');
  if( !u_Lamp0Pos1 || !u_Lamp0Amb1  ) {//|| !u_Lamp0Diff  ) { // || !u_Lamp0Spec  ) {
    console.log('Failed to get the Lamp0 storage locations');
    return;
  }
  // ... for Phong material/reflectance:
  var u_Ke1 = gl.getUniformLocation(gl.program, 'u_Ke1');
  var u_Ka1 = gl.getUniformLocation(gl.program, 'u_Ka1');
  var u_Kd1 = gl.getUniformLocation(gl.program, 'u_Kd1');
  var u_Ks1 = gl.getUniformLocation(gl.program, 'u_Ks1');
...
//	gl.uniform1i(u_Kshiny, 4);							// Kshiny shinyness exponent
  ...
  gl.uniform4f(u_Lamp0Pos1, 6.0, -6.0, 1.0, 1.0);
  // Set its light output:  
  gl.uniform3f(u_Lamp0Amb1,  -0.4, 0.4, -0.4);   // ambient
  gl.uniform3f(u_Lamp0Diff1, 1.0, -1.0, -1.0);   // diffuse
  gl.uniform3f(u_Lamp0Spec1, -1.0, 1.0, -1.0);   // Specular

  // Set the Phong materials' reflectance:
  gl.uniform3f(u_Ke1, 0.0, 0.0, 0.0);        // Ke emissive
  gl.uniform3f(u_Ka1, 0.6, 0.0, 0.0);        // Ka ambient
  gl.uniform3f(u_Kd1, 0.8, 0.0, 0.0);        // Kd diffuse
  gl.uniform3f(u_Ks1, 0.8, 0.8, 0.8);        // Ks specular
//  gl.uniform1i(u_Kshiny, 4);              // Kshiny shinyness exponent
